 ...
 *
 * https://github.com/snowplow/snowplow/wiki/Javascript-Tracker-Core#42-trackscreenview
 *
 * @param {Page} page
 * @param {Object} settings
 * @param {Function} callback
...
Snowplow.prototype.screen = function (screen, settings, callback) {
  var track = screen;
  var trackerCore = createCore(track, settings);
  return this.prepareRequest(track, settings, trackerCore.trackScreenView(
      track.proxy('name'),
      track.proxy('id')
    ), callback);
}
...
Snowplow.prototype.completedOrder = function (track, settings, callback) {
  ...
  var innerCallback = function (err) {
    ...
    // Call the original callback once every request has been sent
    requestCount--;
    if (requestCount <= 0) {
      callback(error);
    }
...
function createCore(track, settings) {
  ...
  trackerCore.setAppId(settings.appId || '');
  trackerCore.setPlatform(platformDict[track.channel()] || 'srv');
  trackerCore.setTrackerVersion('seg-0.1.0');
  trackerCore.setTrackerNamespace(settings.trackerNamespace || 'segmentio-server');
